# Ce document est chiffré à l'aide de l'algorithme de césar
# Vous devrez coder une fonction de DECHIFFREMENT (et non de chiffrement) de césar prenant en paramètres :
#   -Un mot/phrase à déchiffrer (en minuscule)
#   -Une clé étant un nombre entier
# et retournant le mot déchiffré (en minuscule)
#
# La définition de la fonction vous est déjà donné, à vous de l'implémenter
#
# Vous devrez également trouver la valeur de la variable cle. Vous la trouverez peut-être en fouillant le contenu de
# votre pc
#
# Aides :
#   - les fonctions ord(<char>)->int et chr(<int>)->char vous seront utiles
#   - en ascii, le code des lettres minuscules est compris entre 97 (a) et 122 (z)
#   - il ne vous est pas demandé de traiter la ponctuation, mais vous devez traiter les espaces

# A compléter
cle = None


def cesar(word, key):
    # A completer
    return

